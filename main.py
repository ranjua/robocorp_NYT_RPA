...
def main():
    try:
        ...
        nyt_rpa = NYT_RPA(timeout=60, speed=2, auto_close=True, screenshot_directory="output",**config)
        collected_news = nyt_rpa.get_news_from(search_phrase, max_retries=3)
        if collected_news is not None and isinstance(collected_news, int) and collected_news == 2:
            helpers.logger.info("Empty search, with the current parameters.")
            return

        #collected_news = helpers.get_test_data()
        if collected_news is not None and  isinstance(collected_news, list) and len(collected_news) > 0:
            #RPA to write collections in Excel file
            excel_rpa = Excel_RPA()
            filename = helpers.clean_string_for_filename(start_date+" "+end_date+" " + search_phrase + 
                                                         " " + datetime.datetime.now().strftime("%Y%m%d%H%M%S"))
            folder_path = "output"
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
            excel_rpa.save_collection(name=folder_path + "/" + filename + ".xlsx", collection=collected_news)
        else:
